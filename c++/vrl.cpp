...
#define SAMPLE_ORJ 2
#define SAMPLE_SIJ 3
#define SAMPLE_ADJ_SI 4
#define SAMPLE_ADJ_OI 5
...
// advance sampling proportional to the product of the phase functions and the inverse squared distance - simple interpoaltion
Vec getAngularDomainSample_LinearInterpolation(Segment segment, Vec closestPoint, Vec sample, Vec lightDirection, double g, double dist_rayOrigin_closestPoint, double dist_interestPoint_closestPoint, double &pdf) {
	double theta[M], fs[M];
...
Vec mediumRadiance(const Scene& scene, std::vector<Segment> lightRays, Segment cameraRay, RenderSettings settings) {
	...
	for (auto iter = lightRays.begin(); iter != lightRays.end(); ++iter) {
		...
		if (settings.sampling == SAMPLE_UNI) {
			...
			lightSample = getUniformSample(lightRay, pdf_light);
		} else if (settings.sampling == SAMPLE_SIJ || settings.sampling == SAMPLE_ADJ_SI || settings.sampling == SAMPLE_ADJ_OI) {
...
Vec surfaceRadiance(const Scene& scene, std::vector<Segment> lightRays, Segment cameraRay, Vec surfaceColor, Vec surfaceNormal, RenderSettings settings) {
	...
	for (auto iter = lightRays.begin(); iter != lightRays.end(); ++iter) {
		...
		if (settings.sampling == SAMPLE_UNI) {
			...
			lightSample = getAngularDomainSample_LinearInterpolation(lightRay, closestPointL, cameraRay.end, cameraRay.direction, scene.medium->g, dist_lightOrigin_closestPointL, dist_cameraEnd_closestPointL, pdf_light);
		} else if (settings.sampling == SAMPLE_ADJ_OI) {
			lightSample = getAngularDomainSample_OtherInterpolation(lightRay, closestPointL, cameraRay.end, cameraRay.direction, scene.medium->g, dist_lightOrigin_closestPointL, dist_cameraEnd_closestPointL, pdf_light);
		}
